...
public class OkHttpUtil {
     ...
     * 参数三：请求回调
     */
    public static Response postJsonParams(String url, String jsonParams, Map<String, String> headers) {
        String responseBody = "";
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), jsonParams);
        Request request = new Request.Builder()
                .url(url)
     ...
     * 参数三：请求回调
     */
    public static Object[] postParamsJson(String url, String jsonParams) {
        ...
        try {
            ...
            if (response.isSuccessful()) {
                responseObj[0] = true;
                responseObj[1] = response.body().string();
            }
        } catch (Exception e) {
            log.error("okhttp3 post error >> ex = {}", e.getMessage());
        } finally {
